...
def reliability_diagram(ax, obs, fcst, base_rate=None, label="", n_bins=10, debug=False, **kwargs):
    ...
    baserateline = ax.axhline(y=base_rate, color=p.get_color(), label=f"base rate {base_rate:.4f}", linewidth=0.5,
                              linestyle="dashed", dashes=(9, 9))
    baserateline_vertical = ax.axvline(x=base_rate, color=p.get_color(), linewidth=0.5, linestyle="dashed",
                                       dashes=(9, 9))
        ...
        if plabel:
            ...
            for x, y, s in zip(pofd, pody, thresholds):
                if ((x - old_x) ** 2 + (y - old_y) ** 2.) ** 0.5 > sep:
                    ...
                    logging.debug(
                        f"statisticplot.ROC_curve(): tossing {x},{y},{s} annotation. Too close to last label.")
