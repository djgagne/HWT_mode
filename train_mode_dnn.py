...
# Tried from hwtmode.evaluation import brier_skill_score, but AttributeError: 'Tensor' object has no attribute 'mean'
...
def main():
    ...
    if os.path.exists(mcd):
        ...
        logging.error(f"Use ~ahijevyc/bin/hagelslag_obj_pdf.py to make {mcd}")
        sys.exit(1)
    ...
    logging.info(f"dropping {lowconf.sum()}/{len(df)} expert labels with confidence < {confmin} from training set")
    df = df[~lowconf]

    # Labeltime filter
    nlate = (df.labeltime >= freezetime).sum()
    logging.info(f"dropping {nlate}/{len(df)} labeltimes {freezetime} or later")
    ...
    logging.info(f"{train_indices.sum()} ({100.*train_indices.sum()/len(df):.0f}%) training cases Valid_Date < {train_test_split_time}")
    logging.info(f"{test_indices.sum()} ({100.*test_indices.sum()/len(df):.0f}%) test cases Valid_Date >= {train_test_split_time}")
    ...
    value_cnts = True
    if value_cnts:
        logging.info("plotting class histogram")
        df["label"] = labels
    ...
    logging.info(f"{numclasses} classes")
    ...
    if nn:
        ...
        if gridsearch:
            ...
            for i in range(nfit):
                ...
                if not clobber and os.path.exists(model_i):
                    ...
                    history = model.fit(norm_in_data[train_indices], onehotlabels[train_indices], class_weight=class_weight[0], 
                            sample_weight=sample_weight[0], epochs=epochs[0], 
                            validation_data=(norm_in_data[test_indices], onehotlabels[test_indices]), 
                            batch_size=batch_sizes[0], verbose=2)
                    models.append(model)
                    model.save(model_i)
            ...
            statcurves = True
            if statcurves:
                logging.info("reliability diagram, histogram, & ROC curve")
        ...
        logging.info('done normalizing another test set')
        preds = []
    ...
    if rf:
        ...
        logging.info(rf_savedmodel)
        if not clobber and os.path.exists(rf_savedmodel):
            logging.info("loading saved rf")
